...
3. 点击开始，使用手柄完成任务。
4. 手动点击成功、失败或丢弃结束本段。每段已在录制中写好 LeRobot Parquet 和 MP4，结束时只做收尾；另有一个小型 HDF5 保留诊断和时间对齐信息。
...
删除会移除所选片段的 LeRobot 目录（包含 Parquet、视频和元数据）、HDF5 诊断及对应质检记录，释放这些文件占用的空间；已经导出的 Parquet/MP4 数据集是独立副本，继续保留。录制、保存或导出期间不能删除。删除后的原始 episode 编号不会复用，例如删除 `episode_17.h5` 后，新片段仍从 `episode_18.h5` 开始，其他片段不重新编号。原目录和离线质检模式同样支持此操作。

### 保存与导出的区别

- 保存：`<root>/episodes/episode_01/` 已包含标准 LeRobot 的 `data/`、`videos/`、`meta/`，同名 `episode_01.h5` 保留采样时间、关节、夹爪和诊断，供回放与质检。备份请复制整个采集根目录，不能只复制 `.h5`。
- 导出：`<root>/exports/<数据集名>_<时间戳>/<namespace>/<dataset>/`，将勾选片段整理为供 LeRobot/OpenPI 使用的数据集。新片段直接复用 MP4，只修改表格编号和元数据，不重新编码视频。
- 导出编号从 `episode_000000` 开始，按所选原始采集顺序连续排列。

这一方式在重启升级后的采集服务后自动生效，无需更改相机配置或启动命令。旧 HDF5 片段仍然支持，导出时自动采用直接 JPEG → MP4 转换，省掉原先耗时的临时 PNG 步骤；新旧片段可按原有规则共同质检和选择。页面分别显示“正在校验”“正在整理”或“正在转换旧片段”。首次质检仍需要检查视频，导出仍有文件复制和校验开销，耗时随数据量和磁盘速度变化。

流式写入所需的 PyAV、PyArrow、固定版本 LeRobot 已包含在 `bash scripts/setup.sh --data-only` 中。原本可以正常视频导出的完整安装无需重装；若自建采集环境缺少这些依赖，执行该安装命令补齐。
